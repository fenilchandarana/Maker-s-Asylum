...
bool inLDRTest = false;  // Added flag for LDR Test mode
...
void loop() {
  if (inMenu) {
    ...
    if (buttonPressedOnce()) {
      if (selectedItem == 0) {
        ...
        inMenu = false;
        inBuzzerTest = true;
      } else if (selectedItem == 2) {  // LDR Selected
        inMenu = false;
        inLDRTest = true;  // Set LDR test flag
      }
    }
  } 
  ...
  else if (inLDRTest) {  // Check if LDR test mode is active
    drawLDRTest();  // Continuously update the LDR animation
    if (digitalRead(BUTTON1_PIN) == HIGH && digitalRead(BUTTON2_PIN) == HIGH) {
      delay(300);
      while (digitalRead(BUTTON1_PIN) == HIGH || digitalRead(BUTTON2_PIN) == HIGH);
      inMenu = true;
      inLDRTest = false;  // Exit LDR test mode
    }
  }
  delay(100);
}
...
void drawLDRTest() {
  ...
  int glowLevel = map(ldrValue, 0, 1023, 0, 10); // Number of arcs to show
  ...
  do {
    ...
    int cx = 64, cy = 50; // Bulb center

    // Draw bulb
    u8g.drawCircle(cx, cy, 8); // Bulb globe
    u8g.drawLine(cx, cy + 8, cx, cy + 14); // Stem
    u8g.drawLine(cx - 5, cy + 14, cx + 5, cy + 14); // Base

    // Draw arcs above the bulb
    for (int i = 0; i < glowLevel; i++) {
      int y = cy - 5 - i * 2;         // Y-position of each arc
      int w = 4 + i * 2;              // Half-width of each arc increases
