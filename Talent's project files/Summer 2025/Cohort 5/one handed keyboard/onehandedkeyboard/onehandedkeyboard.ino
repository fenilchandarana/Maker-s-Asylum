...
const unsigned long tapTimeout = 500;     // Multi-tap wait
const unsigned long comboWindow = 100;    // Combo detect window
...
void loop() {
  ...
  // If in combo window, check if window is over
  ...
  // Also handle tap timeout for multi-tap
...
void handleComboOrLetters() {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    if (pressedThisCycle[i]) count++;
  }

  // --- Check known combos ---
  if (comboFlags[1] && comboFlags[2] && count == 2) {
    doBackspace();
  } else if (comboFlags[1] && comboFlags[2] && comboFlags[3] && count == 3) {
    toggleCapsLock();
  } else if (comboFlags[1] && comboFlags[2] && comboFlags[3] && comboFlags[4] && count == 4) {
    addSpace();
  } else if (comboFlags[0] && comboFlags[1] && comboFlags[2] && comboFlags[3] && comboFlags[4] && count == 5) {
    addNewline();
  } else {
    // No valid combo → treat as normal taps
    for (int i = 0; i < 5; i++) {
